/*
 * https://github.com/misje/once
 *
 * Copyright (C) 2018 Andreas Misje. All rights reserved.
 */
...
namespace Helper
{
    ...
    template <typename L, int N>
        struct ListLeft
        {
            typedef typename ListAppend<List<typename L::First>, typename
                ListLeft<typename L::Rest, N - 1>::value>::value value;
        };
    template<typename L>
        struct ListLeft<L, 0>
        {
    ...
    template<typename Func, typename ...Args>
        struct CompatibleArgCount<Func, List<Args...>>
        {
            ...
            template<typename T> static T dummy();
             ...
             * */
            template<typename F> static auto test(F func) ->
                decltype(func(dummy<Args>()...), int());
            ...
            enum
            {
                ...
                value = sizeof(test(dummy<Func>())) == sizeof(int)
                     ...
                     * argument: */
                    : static_cast<int>(CompatibleArgCountHelper<Func, List<Args...>,
                            false>::value),
            };
        };
...
std::enable_if<Helper::FuncMeta<Func>::IsFunctor>::type>
{
    ...
    void operator()(Args &&...args) const
    {
        ...
        const int maxArgs = Helper::CompatibleArgCount<Func, typename
            Helper::FuncMeta<Signal>::ArgList>::value;
...
QMetaObject::Connection connect(const Object *sender, Signal signal,
        Functor &&functor)
{
...
template<typename Object, typename Signal, typename Functor>
QMetaObject::Connection connect(const Object *sender, Signal signal, const
        QObject *context, Functor &&functor, Qt::ConnectionType type =
        Qt::AutoConnection)
{
    AutoDisconnecter<Functor, Signal> disconnecter(std::forward<Functor>(functor));
    return *disconnecter.connection = QObject::connect(sender, signal, context,
            disconnecter, type);
}

template<typename Object, typename Signal, typename Functor>
QMetaObject::Connection connect(const Object *sender, Signal signal, int
...
QMetaObject::Connection connect(const Sender *sender, Signal signal, const
        typename Helper::FuncMeta<Slot>::Object *receiver, Slot slot,
        Qt::ConnectionType type = Qt::AutoConnection)
{
    using Receiver = typename Helper::FuncMeta<Slot>::Object;
    SlotAutoDisconnecter<Receiver, Slot, Signal> disconnecter(const_cast<Receiver *>(
                receiver), std::forward<Slot>(slot));
    /* Use receiver as context: */
    return *disconnecter.connection = QObject::connect(sender, signal, receiver,
            disconnecter, type);
}
        ...
        Qt::ConnectionType type = Qt::AutoConnection)
{
    ...
    using Receiver = typename Helper::FuncMeta<Slot>::Object;
    SlotAutoDisconnecter<Receiver, Slot, Signal> disconnecter(const_cast<Receiver *>(
                receiver), std::forward<Slot>(slot), callCount);
    /* Use receiver as context: */
    return *disconnecter.connection = QObject::connect(sender, signal, receiver,
            disconnecter, type);
}
